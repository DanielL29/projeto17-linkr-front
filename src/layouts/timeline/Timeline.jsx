...
export default function Timeline({ publish, title, hashtag, username, pictureUrl, name }) {
    ...
    useInterval(async () => {
        if(isTimeline) {
            const { count } = await getNewPostsQuantity(currentUser.token, posts[0].id ? posts[0].id : 1);
            setNewPosts(count);
        }
    }, POST_ATT_TIME);
        ...
        <TimelineWrapper>
            ...
            {loading ? <PostCard loading={loading} /> :
                posts.length > 0 && typeof posts === "object" ? <Scroll 
                                                                ...
                                                                hashtag={hashtag} username={username} hasMore={hasMore} setHasMore={setHasMore}/> : 
